...
export function VaultStats({ ptBalance, rtBalance, nftCount }: VaultStatsProps) {
  const stats = [
    {
      ...
      description: "Allocation NFTs in your wallet",
    },
    {
      label: "PT Balance",
      value: ptBalance,
      ...
      description: "Principal Tokens — redeemable 1:1 for LaunchToken after unlock",
    },
    {
      label: "RT Balance",
      value: rtBalance,
      ...
      description: "Risk Tokens — settle for USDC after TGE price is set",
    },
  ];
      ...
      {stats.map((stat) => (
        <div key={stat.label} className="relative group overflow-hidden bg-[#101926]/60 backdrop-blur-2xl border border-white/5 rounded-[2rem] p-8 transition-all hover:border-blue-500/30">
          <div className="flex justify-between items-start mb-6">
            ...
            {stat.trend && (
              <span className="text-[10px] font-mono px-2 py-1 rounded-full border text-blue-400 border-blue-400/20 bg-blue-400/5">
                {stat.trend}
              </span>
            )}
          </div>
